<#
.SYNOPSIS
Compares the public scenario API of an already compiled revision using identical synthetic inputs.
.DESCRIPTION
...
untrusted assembly. The same ScriptScaleFixture.cs is compiled for both revisions.
.PARAMETER Root
...
.PARAMETER Probe
Observe three historical failure cases instead of collecting performance samples.
#>
param(
...
$entry = 'global using System; global using System.IO; global using System.Linq; global using System.Collections.Generic; return ScriptScaleFixture.Run(args);'
$trees.Add([Microsoft.CodeAnalysis.CSharp.CSharpSyntaxTree]::ParseText($entry, $parse))
$fixture = Join-Path $Root 'tests/LuckyStarPspToolkit.Formats.SelfTests/ScriptScaleFixture.cs'
...
$compilation = [Microsoft.CodeAnalysis.CSharp.CSharpCompilation]::Create('ScriptScaleProbe', $trees, $refs, $options)
$outputAssembly = Join-Path $Out 'ScriptScaleProbe.dll'
