...
untrusted assembly. The same LicenseScaleFixture.cs is compiled for both revisions.
.PARAMETER Root
...
.PARAMETER Probe
Observe six historical failure cases instead of collecting performance samples.
#>
param(
...
$entry = 'global using System; global using System.IO; global using System.Linq; global using System.Collections.Generic; return LicenseScaleFixture.Run(args);'
$trees.Add([Microsoft.CodeAnalysis.CSharp.CSharpSyntaxTree]::ParseText($entry, $parse))
$fixture = Join-Path $Root 'tests/LuckyStarPspToolkit.Licensing.SelfTests/LicenseScaleFixture.cs'
...
$compilation = [Microsoft.CodeAnalysis.CSharp.CSharpCompilation]::Create('LicenseScaleProbe', $trees, $refs, $options)
$outputAssembly = Join-Path $Out 'LicenseScaleProbe.dll'
