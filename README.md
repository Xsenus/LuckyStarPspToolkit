...
Версия **0.18.0** — C#/.NET 9, инженерная предварительная версия инструмента
...
[Изменения 0.18.0](docs/RELEASE_NOTES_0.18.0_RU.md) ·
...
[Инструкция GitHub](docs/GITHUB_PUBLISH_RU.md). Обычный push запускает проверку и сборку пакетов в Windows/Linux. Push тега `v0.18.0`, совпадающего с `VERSION`, после успешной проверки обеих платформ создаёт **draft prerelease** с ZIP и SHA-256. Перевод не объявляется завершённым автоматически.
...
### Ограничения карты глифов в 0.18.0
...
Лицензии, история резервов и выбор установки используют серверные страницы вместо
полной загрузки базы. Списки возвращают 25 строк (API допускает 1–100), карточка
установок загружается только для выбранной лицензии. Статусы и глобальные счётчики
читаются согласованно. HMAC-курсор связан с фильтрами, endpoint, ревизией базы и исходным
двухминутным снимком времени. При изменении списка React явно возвращает первую страницу.

[Алгоритмы и инструкция](docs/ADMIN_QUERIES_RU.md) ·
[Сравнительный замер](docs/ADMIN_QUERY_PERFORMANCE_RU.md) ·
[Фактические проверки](docs/TEST_REPORT_RU.md).
Сервер и owner-web обновляются вместе, без нового init или смены издателя.
Клиентский протокол и максимальный резерв 168 часов не изменены.
