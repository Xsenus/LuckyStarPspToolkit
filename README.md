...
Версия **0.20.0** — C#/.NET 9, инженерная предварительная версия инструмента
...
В 0.19.2 исправлены чтение настоящих PSP ISO и JSON-отчёт команды `inspect`.
На двух предоставленных образах проверены извлечение, CPK и обратная сборка
без изменения текста. [Точные результаты и ограничения](docs/REAL_GAME_AUDIT_0.19.2_RU.md).
В 0.19.3 запись экспериментального VWF-патча требует явного флага:
дефект клавиатуры имени в PPSSPP ещё не исправлен.
В 0.20.0 проверена расшифровка оригинального NIM EBOOT; профиля записи
изменённого исполняемого файла NIM пока нет.
[Результаты выпуска 0.20.0 и ограничения](docs/VERIFICATION_0.20.0_RU.md).
...
[Изменения 0.19.2](docs/REAL_GAME_AUDIT_0.19.2_RU.md) ·
