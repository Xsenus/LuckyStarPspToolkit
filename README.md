...
Версия **0.15.0** — C#/.NET 9, инженерная предварительная версия инструмента
для анализа и подготовки переводов PSP Lucky Star.

В этой итерации укреплено онлайн-лицензирование: исправлен отсчёт времени после скачков часов,
добавлен защищённый checkpoint между перезапусками, изолированы снимки базы и сокращены
управляемые выделения при изменении лицензий. Часы/дни/годы/бессрочный доступ, лимит установок,
активация, продление, приостановка и отзыв сохранены.
...
[Модель защиты](docs/LICENSE_SECURITY_RU.md) · [Протокол](docs/LICENSE_API.md) · [Обновление 0.14 → 0.15](docs/LICENSE_MIGRATION_RU.md) ·
[Исправления](docs/LICENSE_HARDENING_RU.md) · [Замеры](docs/LICENSE_PERFORMANCE_RU.md)
...
[Изменения 0.15.0](docs/RELEASE_NOTES_0.15.0_RU.md) ·
...
[Инструкция GitHub](docs/GITHUB_PUBLISH_RU.md). Обычный push запускает проверку и сборку пакетов в Windows/Linux. Push тега `v0.15.0`, совпадающего с `VERSION`, после успешной проверки обеих платформ создаёт **draft prerelease** с ZIP и SHA-256. Перевод не объявляется завершённым автоматически.
...
### Ограничения карты глифов в 0.15.0
