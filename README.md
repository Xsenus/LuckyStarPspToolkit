...
Версия **0.17.0** — C#/.NET 9, инженерная предварительная версия инструмента
для анализа и подготовки переводов PSP Lucky Star.

В 0.17.0 добавлены **React-панель владельца** с password/TOTP/recovery и отдельный,
...
[Изменения 0.17.0](docs/RELEASE_NOTES_0.17.0_RU.md) ·
...
[Инструкция GitHub](docs/GITHUB_PUBLISH_RU.md). Обычный push запускает проверку и сборку пакетов в Windows/Linux. Push тега `v0.17.0`, совпадающего с `VERSION`, после успешной проверки обеих платформ создаёт **draft prerelease** с ZIP и SHA-256. Перевод не объявляется завершённым автоматически.
...
### Ограничения карты глифов в 0.17.0
...
[Резерв и сессии владельца](docs/RESERVE_SESSION_HARDENING_RU.md), [измерения](docs/RESERVE_PERFORMANCE_RU.md), [проверки](docs/TEST_REPORT_RU.md). Комплект остаётся owner-only.
