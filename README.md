...
Версия **0.12.0** — C#/.NET 9, инженерная предварительная версия инструмента
для анализа и подготовки переводов PSP Lucky Star.

В этой итерации исправлены воспроизведённые ошибки сценарного парсера и пересборки:
ложный отказ no-op, чтение checksum как текста, переполнение metadata offsets.
Пересчёт переходов переведён на binary-search индекс; лимиты проверяются до
выделения текстовых массивов и итогового буфера. Добавлены 13 групп регрессий
и измерение на 8192 диалогах с побайтовым сравнением результата версий.
...
[Изменения 0.12.0](docs/RELEASE_NOTES_0.12.0_RU.md) ·
[Алгоритм сценариев](docs/SCRIPT_CODEC_RU.md) ·
[Измерения](docs/PERFORMANCE_RU.md)
...
Новые границы сценария: 64 МиБ на файл, 1 000 000 глифов на поле и
8 000 000 глифов совокупно, настраиваемые через FileLimits. Дубли текстовых
адресов, пересечения метаданных и невыравненные targets отвергаются.
Без изменений корректный сценарий возвращается побайтно, включая padding.
Переходы внутрь действительно изменённой записи не угадываются и блокируют сборку.

## Публикация на GitHub

[Инструкция GitHub](docs/GITHUB_PUBLISH_RU.md). Обычный push запускает проверку и сборку пакетов в Windows/Linux. Push тега `v0.12.0`, совпадающего с `VERSION`, после успешной проверки обеих платформ создаёт **draft prerelease** с ZIP и SHA-256. Перевод не объявляется завершённым автоматически.
...
### Ограничения карты глифов в 0.12.0
