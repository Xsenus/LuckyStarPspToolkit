# Лицензирование 0.16.0: инструкция владельца

## Что вы передаёте клиенту

Только клиентский ZIP из `artifacts/releases/0.16.0/` и отдельный файл ключа, например `artem-trial.txt`.
...
Обычный режим требует постоянно доступный сервер. В 0.16.0 владелец может отдельно выдать подписанный
...
В папке `project` нужны .NET 9 SDK, Python 3.11+, Node.js 22+ и зависимости проверки:
...
Результат: `artifacts/owner-releases/0.16.0/`. В каждой платформе отдельные каталоги `server/`, `admin/` и `web/`. React-вход настраивается по [ADMIN_WEB_RU.md](ADMIN_WEB_RU.md).
...
Клиентский ZIP появится в `artifacts/releases/0.16.0/`. Публичная конфигурация включена в ресурс сборки,
...
```powershell
.\artifacts\releases\0.16.0\win-x64\lsptool.exe license build-info
```
...
безостановочной ротации, HSM и HA в 0.16.0 нет.
