# Лицензирование 0.14.0: инструкция владельца

## Что вы передаёте клиенту

Только клиентский ZIP из `artifacts/releases/0.14.0/` и отдельный файл ключа, например `artem-trial.txt`.
**ALL_IN_ONE, исходники, Git bundle, LicenseAdmin, LicenseServer и папка authority остаются у вас.**
Полный комплект — ваша поставка разработчика, а не клиентский дистрибутив.

Эта версия требует ваш постоянно доступный сервер. Без сети новый запуск защищённых команд запрещён.
...
В папке `project` нужны стабильный .NET 9 SDK, Python 3.11+ и зависимости проверки:
...
Результат: `artifacts/owner-releases/0.14.0/`. В каждой платформе отдельные каталоги `server/` и `admin/`.
Публикация проходит общие тесты, лицензирование и сетевой сценарий на тестовом издателе.
**В этом исходном комплекте готовых EXE нет; штатный .NET 9 publish здесь не был выполнен.**

## 2. Создать издателя на Ubuntu VPS

Нужны ваш домен с DNS, действующий TLS-сертификат и Nginx. Пример ниже предполагает, что Linux-сборки
...
Не отключайте проверку сертификата в клиенте. Эти команды/шаблоны подготовлены, но ваш VPS здесь не настраивался.
...
Клиентский ZIP появится в `artifacts/releases/0.14.0/`. Публичная конфигурация включена в ресурс сборки,
...
```powershell
.\artifacts\releases\0.14.0\win-x64\lsptool.exe license build-info
```
...
Остановите единственный сервер, скопируйте весь закрытый каталог authority и отдельно passphrase, затем запустите.
...
Сервер откажется стартовать при неверной подписи базы или отсутствующем файле.
...
безостановочной ротации, HSM и HA в 0.14.0 нет.
