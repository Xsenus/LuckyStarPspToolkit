# Лицензирование 0.19.0: инструкция владельца

## Что вы передаёте клиенту

Только клиентский ZIP из `artifacts/releases/0.19.0/` и отдельный файл ключа, например `artem-trial.txt`.
...
Обычный режим требует постоянно доступный сервер. В 0.19.0 владелец может отдельно выдать подписанный
...
Результат: `artifacts/owner-releases/0.19.0/`. В каждой платформе отдельные каталоги `server/`, `admin/` и `web/`. React-вход настраивается по [ADMIN_WEB_RU.md](ADMIN_WEB_RU.md).
...
Клиентский ZIP появится в `artifacts/releases/0.19.0/`. Публичная конфигурация включена в ресурс сборки,
...
```powershell
.\artifacts\releases\0.19.0\win-x64\lsptool.exe license build-info
```
...
безостановочной ротации, HSM и HA в 0.19.0 нет.
