# Лицензирование 0.15.0: инструкция владельца

## Что вы передаёте клиенту

Только клиентский ZIP из `artifacts/releases/0.15.0/` и отдельный файл ключа, например `artem-trial.txt`.
...
Результат: `artifacts/owner-releases/0.15.0/`. В каждой платформе отдельные каталоги `server/` и `admin/`.
...
Клиентский ZIP появится в `artifacts/releases/0.15.0/`. Публичная конфигурация включена в ресурс сборки,
...
```powershell
.\artifacts\releases\0.15.0\win-x64\lsptool.exe license build-info
```
...
безостановочной ротации, HSM и HA в 0.15.0 нет.
