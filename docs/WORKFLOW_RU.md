> **Изменение 0.15.0:** все рабочие команды требуют онлайн-лицензию. Сначала [инструкция владельца](LICENSE_OWNER_RU.md) и [активация](LICENSE_CUSTOMER_RU.md). Сборка без `--license-trust` остаётся заблокированной. Старое standalone demo требует уже активированный CLI; `scripts/license_integration.py` проверяет полный сценарий на отдельном тестовом издателе.
...
Расшифровка разрешена только для подтверждённой ревизии `ULJM05752`.
VWF-профиль содержит 133 проверяемые MIPS-замены, но пока не прошёл
игровую проверку: в PPSSPP пропали символы клавиатуры имени.
Подробнее: [EBOOT_VWF_PATCH_RU.md](EBOOT_VWF_PATCH_RU.md).
...
## 12. Проверить необходимость изменения EBOOT

Если перевод помещается в исходные размеры сценариев, оставьте оригинальный
EBOOT: именно с ним проверены две короткие русские реплики в PPSSPP.
Если `workspace-build` создал план с изменёнными размерами, отдельно проверьте
исходный EBOOT и примените только план к расшифрованному чистому ELF:
...
Проверяются точный SHA-256 исходной ревизии, таблица блоков 2 КиБ и script
heap. Если крупнейший сценарий превышает исходные `0x236000` байт, пара
MIPS `lui/addiu` увеличивается с правильной обработкой знакового immediate.
Изменённый EBOOT нужно отдельно проверить в игре; два коротких пробных
диалога не проверяют этот путь.

Совмещённая запись VWF и плана остаётся только для исследования:
...
Итог обеих команд — расшифрованный ELF, не заново подписанный retail PRX.
Экспериментальный VWF-результат не включайте в клиентский ISO до устранения
дефекта и повторной проверки. Подробности: [EBOOT_VWF_PATCH_RU.md](EBOOT_VWF_PATCH_RU.md).
